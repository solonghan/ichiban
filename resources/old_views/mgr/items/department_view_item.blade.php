...
<script>
        $(document).ready(function(e){
           ...
           $(document).on('click', '.specialty_list-item-btn', function(event) {
                ...
                location.href = '{{env('APP_URL')}}/mgr/committeeman/specialty_list/' + id;
            });

             $(document).on('click', '.academics_list-item-btn', function(event) {
                var id = $(this).closest('tr').data('id');
                location.href = '{{env('APP_URL')}}/mgr/committeeman/academics_list/' + id;
            });
            ...
            $(".switch_toggle_com").on('change', function () {
                ...
                $.ajax({
                    type: "POST",
                    url: '{{env('APP_URL')}}/mgr/committeeman/switch_toggle_com',
                    data: {
                        _token: '{{ csrf_token() }}',
                        id: $(this).data('id'),
                        status: status
