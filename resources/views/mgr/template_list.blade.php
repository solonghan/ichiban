...
@section('content')
    ...
    <div class="row">
        <div class="col-12">
        {{-- @if ($type=='search')
             <p>查詢條件  姓名:{{$search_name}}， 服務單位:{{$search_now_unit}} {{$have}}，職稱:{{$search_title}}，專門專長:{{$search_specialty}}，學術專長:{{$search_academic}}  ，最後異動時間:{{$last_date}} ，外審專家建立之所屬系所:{{$search_department_id}}</p>
        @elseif($type=='output')   
