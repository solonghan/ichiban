...
class MemberController extends Mgr
{
	...
	public function data(Request $request){
		...
		// $data =$data->paginate($page_count, ['*'], 'page', $page);
		// print_r($data);exit;
	...
	public function add(Request $request){
		...
		if ($request->isMethod('post')) {
			...
			if (Member::where('ID_number', $formdata['ID_number'])->count() > 0){
				$this->js_output_and_back('此身分證號已存在');
				exit();
			}
	...
	public function edit(Request $request, $id){
		...
		if ($request->isMethod('post')) {
			...
			// print_r($new_department);exit;
			if($new_department==''){
				// $formdata['department_id']
				$num=1;
				// $department_data['department_id']=$formdata['my_department_id'];
				// $formdata['department_id']=array($formdata['my_department_id']);
				$department_data['department_id']=0;
				$formdata['department_id']='';
			}else{
	...
	public function department_view_data(Request $request){
		$member_id=Auth::guard('mgr')->user()->id;
		...
		if($privilege_id==1){
			if($search!=''){
				...
				$total=Committeeman::with('specialty')
				->with('academic')
				// ->with('writer')
				->with('get_member')
				// ->where('member_id',$member_id) //帳號本人才顯示
			...
			if($search!=''){
				...
				select('committeemen.*','members.my_department_id','members.department_id','academics.writer_id','specialties.title_id','academics.title')
				// select('committeemen.*','members.department_id')
				...
				->where(function($query) use ($search_s_id,$search_a_title,$search,$search_J_id,$search_Service_id,$member_id,$my_department_id,$department_id) {
						...
						$query->orWhere('committeemen.username', 'like', '%'.$search.'%');
						$query->orWhere('committeemen.phone', 'like', '%'.$search.'%');
						$query->orWhere('committeemen.email', 'like', '%'.$search.'%');
						$query->orWhere('committeemen.url', 'like', '%'.$search.'%');
						$query->orWhere('committeemen.now_unit', 'like', '%'.$search.'%');

						// $query->where('member_id',$member_id); //帳號本人才顯示
						// $query->orwhere('my_department_id',$my_department_id); 
				...
				// 	$query->orwhere('writer_id',$member_id);
				// })
				->with('get_member')
				...
				// 				// $query->orWhere('members.mydepartment_id','like', '%'.$d_id.'%');
				// 				$query->orWhere('members.my_department_id',$d_id);
				// 			}
				// 		}
				...
				$total=Committeeman::
				// select('committeemen.*','academics.writer_id',)
				select('committeemen.*','members.my_department_id','members.department_id','academics.writer_id')
				// select('committeemen.*','members.department_id')
				->leftJoin('academics', function($leftJoin) use($academic_list)
										{
											$leftJoin->on('academics.committeeman_id', '=', 'committeemen.id');
			...
			// print 'member_id:';
			// print_r($member_id);
			// print ' my_department_id:';
			// print_r($my_department_id);
			// print ' total:';
			// print_r($total);exit;
			// dd(DB::getQueryLog());exit;
		}
		...
		if($privilege_id==1){
			...
			if($search!=''){
				$data=Committeeman::
			// select('committeemen.*','members.my_department_id','members.department_id')
			select('committeemen.*','members.my_department_id','members.department_id','academics.writer_id','specialties.title_id','academics.title')
			...
			// ->with('writer')
			->with('get_member')
			// ->whereHas('get_member', function($query) use($my_department_id) {
			// 	$query->orwhere('members.my_department_id',$my_department_id);
			// })
			// ->where('member_id',$member_id) //帳號本人才顯示
			// ->orwhere('academics.writer_id',$member_id)
			->where('academics.deleted_at',NULL)
		...
		// }
			...
			if (
				...
				$status = '
					<div class="form-check form-switch">
						<input class="form-check-input switch_toggle_com" data-id="'.$item->id.'" type="checkbox" role="switch" '.(($item->status=='on')?'checked':'').'>
					</div>
				';
			}else{
				if($item->status == 'off') $status = '<span class="">關閉</span>';
			...
			$html .= view('mgr/items/department_view_item', [
				'item'      => array(
					"id"         => $item->id,
					"data"       => $obj,
					"priv_edit"  => $priv_edit,
