...
>>>>>>> b4f505987eda3949c0fbb43cbd59ae3669fec6c6
use App\Models\Dist;
use Auth;
class BaseController extends Controller
{
	...
	public function process_post_data($param, $request, $langs = false, $required_enabled = true, $required_alarm_type = 'js'){
		...
		$should_continue_field = ["", "button", "array", "header", "select_multi", "img_multi", "plain", "empty", "file"];
		if (!$required_enabled) {
		...
		foreach ($param as $item) {
			...
			if ($required_enabled && $required) {
				if ($type == 'select_multi' && (is_null($val) || !is_array($val) || count($val) <= 0)) {
					...
					// print_r($item[1]);exit;
					if($field!='now_unit_s' || $field!='old_unit_s'){
						if ($required_alarm_type == 'js') {
	...
	public function city(){
		$action = Request::post('action')??'dist';
		$id = Request::post('id');

		if ($action == 'dist') {
			$this->output(TRUE, "success", ['data'=>Dist::where(['city_id'=>$id])->get()->toArray()]);
		}
	}
}
