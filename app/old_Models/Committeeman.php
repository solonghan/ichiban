    ...
    public static function get_academic($title,$id){
        ...
        foreach($academic_committeeman_id as $s_committeeman_id){
            ...
            foreach($academic_data_all as $a_title){
                // print_r($a_title['title']);
                $writer_data=Member::find($a_title->writer_id);
                $academic_list_data=Academic::where('title','like', '%'. $a_title['title'].'%')->where('committeeman_id',$id)->first();
                // find($s_title->title_id);
                // print " ID: ";
                // print_r($academic_list_data->id);
                // print ' TITLE: ';
                // print_r($academic_list_data->title);
                // print ',,,';
                if(isset($academic_list_data)){
